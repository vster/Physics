...
syms x ksi(x) m hp U0 E 
eqn=diff(ksi,x,2)==(2*m/hp^2)*(U0-E)*ksi
solksi=dsolve(eqn)
% C4*exp((2^(1/2)*x*(-m*(E - U0))^(1/2))/hp) + C5*exp(-(2^(1/2)*x*(-m*(E - U0))^(1/2))/hp)
pretty(solksi)
% ksi = exp(-eta*x) and exp(+eta*x)
...
% If x=x0
% ksi1(x0)=ksi2(x0) => B*cos(k*x)=A*exp(-eta*x)
...
% Put y0=sqrt((2*m/hp^2)*U0)x0 and ksi=k*x0
% then
